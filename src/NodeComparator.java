...
public class NodeComparator implements Comparator<KeywordTrie.Node>
{
    ...
    public NodeComparator(String query)
    {
        ...
        queryNode = Main.trie.findEntry(query);
        if (queryNode != null)
        {
    ...
    public double getSuggestedRank(KeywordTrie.Node node)
    {
        ...
        try
        {
            String[] otherNodeWords = node.getWordRepresented().split(" ");
            double wcf_score = wcf_app.compare(lastQueryWord, otherNodeWords[queryWordNumber]);
            if (wcf_score == -1)
                wcf = 0;
            else
                wcf = Math.log(wcf_score);
        } catch (IOException e)
        {
            e.printStackTrace();
        }
        ...
        //System.out.println(node.getWordRepresented() + " and " + queryNode.getWordRepresented() + " has a score of " + (freq + wcf + mod) / (1 - Math.min(Math.min(freq, wcf), mod)));
